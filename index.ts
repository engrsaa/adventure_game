...
// Game Variables
const enemies:{[key:string]:number}={'Skeleton':5,'Zombie':10,'Warrior':15,'Assasin':20};
// let killedEnemies=[];
let score=0;
...
,   healthPotionDropChance = 50 //it is a percentage
,   running=true;
// welcome msg
...
Game:while(running){
    ...
    while(enemyHealth>0){
        ...
        if(userAction.opt==='Attack'){
            let demageDealt = Math.ceil(Math.random()*attackDemage)
            ,   demageTaken = Math.ceil(Math.random() * attackDemage);
